...
<div id="galleryContainer">
    ...
    <?php if($gallery['pages']): ?>
        <form method="post" id="imgerase" action="eraseImages">
        <div id="imageGallery">
        <?php foreach($gallery['images'] as $zdjecie): ?>
            <div class="galleryImage">
                ...
                <div>
                    ...
                    <p><b>USUŃ:</b> <input type="checkbox" id="imgerase" name="savedImages[]" value=<?=$zdjecie['_id']?>></p>   
                    <?php if(isset($_SESSION['isLogged'])&&$_SESSION['isLogged']==='yes'):?>
                        <?php if($zdjecie['isPublic']==='no'):?>
                           <p style="color:red;">ZDJĘCIE PRYWATNE</p>
                        <?php endif?>
                    <?php endif?>           
                </div>
            </div>
    ...
    <a <?php if($i==$gallery['currentPage']) echo 'style="background-color: #0f00dd; color:white;"'?> href="session_gallery?page=<?=$i?>"><?=$i?></a>
    <?php endfor?>
</div>
<input type="submit" id="imgerase" value="Usuń zaznaczone zdjęcia"/>
</form>
<?php else: ?>
